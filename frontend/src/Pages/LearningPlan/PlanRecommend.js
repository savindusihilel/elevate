  ...
  useEffect(() => {
    const fetchFilteredPosts = async () => {
      try {
        ...
        const userSkillsResponse = await axios.get(`http://localhost:8080/user/${userID}/skills`);
        const userSkills = userSkillsResponse.data.map(skill => skill.toUpperCase());

        const postsResponse = await axios.post('http://localhost:8080/learningPlan/filterBySkills', userSkills);
