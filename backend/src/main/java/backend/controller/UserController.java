...
public class UserController {
    ...
    public ResponseEntity<?> newUserModel(@RequestBody UserModel newUserModel) {
        ...
        // Ensure skills are initialized if not provided
        if (newUserModel.getSkills() == null) {
            newUserModel.setSkills(new ArrayList<>());
        }
    ...
    public List<String> getUserSkills(@PathVariable String id) {
        return userRepository.findById(id)
                .map(UserModel::getSkills)
